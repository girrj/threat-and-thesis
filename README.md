...
M4xHermes 텔레그램 대화에서도 `Threat & Thesis`를 언급해 작업할 수 있습니다. `분석해`, `검토해`는 읽기 전용이며, `디자인 수정해`, `카테고리 바꿔`, `수집기 고쳐서 적용해`처럼 실행을 명시하면 전용 작업 폴더에서 수정·테스트·배포까지 진행합니다. 3시간 예약 작업은 계속 콘텐츠 갱신만 수행하며 디자인이나 코드 구조를 임의로 바꾸지 않습니다.
