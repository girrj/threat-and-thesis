...
- `threat-and-thesis-3h` 예약 실행은 아래 3시간 갱신 절차만 수행한다. UI, 스키마, 의존성, 수집 코드, 운영 정책을 임의로 바꾸지 않는다.
- 인증된 사용자가 M4xHermes 텔레그램 대화에서 직접 디자인, 코드, 카테고리, 수집기, 운영 설정의 수정·적용·배포를 요청하면 해당 범위의 변경을 수행할 수 있다. 이때 `skills/threat-and-thesis/SKILL.md`의 대화형 유지보수 절차를 따른다.
- `분석`, `검토`, `의견`, `상태` 요청은 읽기 전용으로 처리한다. `수정해`, `바꿔`, `적용해`, `배포해`처럼 실행을 명시한 요청은 검증 후 구현·배포까지 수행한다. 삭제, 인증 변경, 범위가 크게 갈리는 선택은 먼저 확인한다.
...
6. 오늘 날짜의 `content/daily/YYYY-MM-DD.json`을 `selectionMode: new-only`로 만든다. 오늘 처음 공개한 항목만 같은 날짜 파일에 누적하고, 전날까지 한 번이라도 발행한 항목은 오늘 순위에 다시 넣지 않는다. 새 항목이 없는 날도 빈 발행본을 만들어 전날 자료가 최신 화면에 남지 않게 한다. 지난 날짜 스냅샷은 사실 오류를 바로잡는 경우 외에는 수정하지 않는다.
7. `security`, `ai-security`, `security-paper`, `ai-paper`, `technology`를 섞은 전체 순위를 만들지 않는다. 오늘 새로 공개한 항목만 각 카테고리 안에서 최대 10건까지 별도로 순위화한다. 빈 카테고리는 빈 배열로 둔다. 긴급성·실제 악용·출처 신뢰도·신선도·실무 영향·연구 기여를 함께 고려하고, 각 항목에 출처로 설명 가능한 `reason`을 한 문장으로 쓴다.
8. `new-only` 발행본의 모든 항목은 `previousRank: null`, `status: new`를 사용한다. 이전 날짜 자료의 순위 유지·재진입·교체는 수행하지 않는다.
...
11. 검증이 모두 통과한 경우에만 변경 내용을 보고한다. 커밋·푸시는 사용자가 해당 작업에 명시적으로 요청했을 때만 수행한다. 오늘의 빈 `new-only` 발행본을 처음 만드는 것은 공개 변경이지만, 이미 오늘 발행본이 있고 새 자료나 정정이 없으면 빈 커밋을 만들지 않는다.
